...
const FloatingSocials = () => {
  const socials = [
    ...
    },
    {
      ...
      color: 'hover:bg-gradient-to-tr hover:from-[#f9ce34] hover:via-[#ee2a7b] hover:to-[#6228d7] hover:shadow-pink-500/30',
      icon: (
      ...
      {socials.map((social) => (
        <a
          ...
          className={`
            ...
            bg-white/80 backdrop-blur-md text-forest border border-forest/10
            shadow-[0_8px_30px_rgba(27,67,50,0.08)] transition-all duration-500 hover:-translate-x-1.5 hover:text-white
            ${social.color} hover:scale-110 group cursor-pointer
          `}
        ...
        >
          ...
          <span className="
            absolute right-full mr-3 px-3 py-1.5 rounded-xl bg-forest text-white text-[9px] font-black uppercase tracking-widest
            opacity-0 pointer-events-none translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300
            shadow-lg border border-white/10 whitespace-nowrap
          ">
            {social.name === 'WhatsApp' ? 'Chat with us' : `Follow ${social.name}`}
          </span>
        </a>
      ))}
    </div>
