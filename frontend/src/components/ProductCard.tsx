    ...
    <div className="group bg-white rounded-[2.5rem] overflow-hidden shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] transition-all duration-700 hover:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.12)] flex flex-col w-full relative cursor-pointer border border-transparent hover:border-gold/20">
      ...
      <div className="px-6 pb-6 md:px-8 md:pb-8 mt-auto">
        <button
          ...
          className="w-full bg-forest text-white py-4 md:py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] transition-all duration-500 hover:bg-cream hover:text-forest hover:shadow-[0_15px_30px_-5px_rgba(212,175,55,0.4)] hover:-translate-y-1 active:scale-[0.98]"
        >
          WhatsApp Order
        </button>
        {/* Acquire button to navigate to product details */}
        <Link href={`/product/${product.id}`} className="w-full mt-3 text-center bg-cream text-forest py-3 rounded-xl font-black text-[10px] uppercase tracking-[0.3em] hover:bg-forest hover:text-white transition-colors duration-300 block">
          Acquire
        </Link>
      </div>
